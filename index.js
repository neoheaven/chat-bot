...
db.ref(`messages/${CHAT_CHANNEL}`)
  ...
  .on("child_added", async (snap) => {
    ...
    const msg = snap.val();
    if (!msg) return;
    if (msg.fromDiscord) return; // 무한루프 방지
...
setTimeout(() => {
  isInit = false;
  console.log("✅ Firebase 리스너 준비 완료 — 앱 메시지를 Discord로 전달합니다");
}, 3000);
...
client.on(Events.MessageCreate, async (message) => {
  ...
  // message.author.bot 만 체크하면 웹훅도 bot=true 라 걸리지만,
  // webhookId 를 먼저 확인해서 명확하게 구분
      ...
      try {
        ...
        if (ref) {
          replyTo = {
            key:  `discord_${ref.id}`,
            nick: ref.webhookId
              ? ref.author.username          // 채팅앱에서 온 웹훅이면 username 사용
              : `${ref.author.displayName} 🎮`,
            text: (ref.content || "").slice(0, 80),
          };
        }
      } catch (_) { /* 메시지 조회 실패해도 계속 진행 */ }
    }
